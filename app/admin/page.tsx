...
export default function AdminLoginPage() {
  ...
  // Prefetch the dashboard route so navigation is instant after login
  useEffect(() => {
    try {
      router.prefetch('/admin-s4u-dashboard');
    } catch (error) {
      // Ignore prefetch issues in dev; routing will still work on navigation.
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[AdminLogin] Prefetch failed', error);
      }
    }
  }, [router]);
